...
- `artifacts/api-server/src/lib/auth-state.ts` — Supabase-backed multi-file Baileys credential/key persistence
- `artifacts/api-server/supabase/20260908_knot_broadcast_backend.sql` — queue claim and session schema
- `artifacts/api-server/supabase/20260908_knot_linking.sql` — pairing-code and per-user auth-file schema
- `.local/conversation-workspace/files/knot-multi-user/whatsapp-link.js` — dashboard QR/pairing UI
...
- Baileys auth credentials and Signal keys are stored as per-user Supabase auth files so sessions survive API restarts.
...
Run all three Knot Supabase migrations before exercising upload, pairing, or worker routes. The worker intentionally marks jobs failed when no connected WhatsApp session exists.
